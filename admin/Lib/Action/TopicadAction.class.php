...
class topicadAction extends BaseAction{
    ...
    function index() {
        ...
        for($i=0; $i<count($ad_list); $i++) {
            ...
            switch($ad_list[$i]['position']) {
                case '1' :
                    $ad_list[$i]['positionText'] = '左上';
                    break;
                case '2' :
                    $ad_list[$i]['positionText'] = '右上';
                    break;
                case '3' :
                    $ad_list[$i]['positionText'] = '左下';
                    break;
                case '4' :
                    $ad_list[$i]['positionText'] = '右下';
                    break;
            }
            //status
    ...
    public function editsave() {
        ...
        if($adinfo!==false) {
            updateCache("ls_found_ad");//更新专题广告位缓存
            $this->success('保存成功','admin.php?m=Topicad&a=index');
        } else {
            $this->error('添加失败');
        }
    }
    ...
    public function addsave() {
        ...
        if($adinfo) {
            updateCache("ls_found_ad");//更新专题广告位缓存
            $this->success('添加成功','admin.php?m=Topicad&a=index');
        } else {
            $this->error('添加失败');
        }
    }
