    ...
    static function write($message,$level=self::ERR,$type='',$destination='',$extra='') {
        ...
        if(self::FILE == $type) { // 文件方式记录日志
            if(empty($destination))
                switch($level){
                    case self::ERR:
                        $dir = ROOT_PATH.'/Logs'.'/Error';
                        if(!is_dir($dir)){
                            mkdir($dir,0777,true);
                        }
                        $destination = $dir.'/'.MODULE_NAME.date('Ymd').'.log';
                        break;
                    case self::WARN:
                        $dir = ROOT_PATH.'/Logs'.'/Warning';
                        if(!is_dir($dir)){
                            mkdir($dir,0777,true);
                        }
                        $destination = $dir.'/'.MODULE_NAME.date('Ymd').'.log';
                    ...
                    case self::DEBUG:
                        ...
                        break;
                    default:
                        $destination = C('LOG_PATH').date('y_m_d').'.log';  //系统默认日志，记录在项目"Runtime/Logs"目录
                }
            //检测日志文件大小，超过配置大小则备份日志文件重新生成
            if(is_file($destination) && floor(C('LOG_FILE_SIZE')) <= filesize($destination) )
                rename($destination,dirname($destination).'/'.time().'-'.basename($destination));
